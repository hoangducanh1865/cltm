...
def reload_evaluation_ppl(params, suffix=""):
	with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(
		...
		saver_ppl = tf.compat.v1.train.import_meta_graph("model/" + params['reload_model_dir'] + "model_ppl/model_ppl-1.meta")
		saver_ppl.restore(session_ppl, tf.compat.v1.train.latest_checkpoint("model/" + params['reload_model_dir'] + "model_ppl/"))
...
def main(args):
	...
	if args.reload:
		...
		if reload_ir:
			...
			saver_ir = tf.compat.v1.train.import_meta_graph("model/" + args.reload_model_dir + "model_ir/model_ir-1.meta")
			saver_ir.restore(sess_ir, tf.compat.v1.train.latest_checkpoint("model/" + args.reload_model_dir + "model_ir/"))
