...
def linear(input, output_dim, input_bw=None, scope=None, stddev=None, W_initializer=None):
	...
	if W_initializer is None:
		if stddev:
			norm = tf.compat.v1.random_normal_initializer(stddev=stddev)
		else:
			norm = tf.compat.v1.random_normal_initializer(
				stddev=np.sqrt(2.0 / input.get_shape().as_list()[1])
			)
	...
	b_bw = tf.compat.v1.get_variable('b_bw', [output_dim], initializer=const)

	input_bw_logits = None
	if input_bw is None:
		input_logits = tf.matmul(input, w) + b
	else:
		input_logits = tf.matmul(input, w) + b
		input_bw_logits = tf.matmul(input_bw, w) + b_bw
...
def linear_TL(input, output_dim, input_old=None, scope=None, stddev=None, U_pretrained=None, bias_sharing=True):
	...
	if not bias_sharing:
		b_old = tf.compat.v1.get_variable('b_old', [len(input_old), output_dim], initializer=const)

	input_logits = tf.matmul(input, U) + b
	input_old_logits = []
	if input_old:
		for i, input_old_temp in enumerate(input_old):
			if bias_sharing:
				input_old_temp_logits = tf.matmul(input_old_temp, U) + b
			else:
				input_old_temp_logits = tf.matmul(input_old_temp, U) + b_old[i]
...
def linear_TL_reload(input, output_dim, input_old=None, scope=None, stddev=None, W_initializer=None, 
	...
	if input_old is None:
		...
		if b_old_reload is None:
			input_old_logits = tf.matmul(input_old, w) + b
		else:
			input_old_logits = tf.matmul(input_old, w) + b_old
...
def masked_sequence_cross_entropy_loss(
	...
	if norm_by_seq_lengths:
		loss = loss / tf.cast(seq_lengths, tf.float32)
	...
	def __init__(self, x, y, x_old, x_old_doc_ids, seq_lengths, 
		...
		with tf.device('/cpu:0'):
			...
			if not params.bias_sharing:
				bias_old = tf.compat.v1.get_variable(
					'bias_old',
			...
			if params.use_embeddings_prior:
				if params.lambda_embeddings == "manual":
					self.embeddings_lambda_list = tf.compat.v1.get_variable(
						'embeddings_lambda_list',
						initializer=lambda_embeddings_list,
						trainable=False
					)
				elif params.lambda_embeddings == "automatic":
					embeddings_lambda_list = tf.compat.v1.get_variable(
				...
				for i, W_embeddings in enumerate(W_embeddings_matrices_list):
					W_prior = tf.compat.v1.get_variable(
			...
			if params.sal_loss:
				if params.sal_gamma == "manual":
					self.sal_gamma = tf.compat.v1.get_variable(
						'sal_gamma',
					...
					"""
					sal_gamma = tf.compat.v1.get_variable(
						'sal_gamma_unclipped',
						[params.old_dataset_size, 1],
						initializer=tf.compat.v1.constant_initializer(params.sal_gamma_init),
						trainable=True
					)
			...
			if params.ll_loss:
				if params.ll_lambda == "manual":
					self.ll_lambda_list = tf.compat.v1.get_variable(
						'll_lambda_list',
					...
					"""
					ll_lambda_list = tf.compat.v1.get_variable(
				...
				if params.projection:
					...
					for i, ll_temp_init in enumerate(params.ll_lambda_init):
						ll_proj_temp_W = tf.compat.v1.get_variable(
						...
						ll_proj_temp_U = tf.compat.v1.get_variable(
		...
		for i, embeddings_old in enumerate(self.embeddings_old_list):
			...
			indices_old_temp = tf.stack([
				tf.range(batch_size),
				tf.cast(seq_lengths_old_list[i], tf.int32)
			], axis=1)
		...
		if params.pretraining_target:
			pretrain_step = tf.compat.v1.Variable(0, trainable=False)
		...
		if params.sal_loss:
			...
			for i, logits_old_temp in enumerate(self.logits_old):
				...
				if params.sal_gamma == "automatic":
					...
					sal_gamma_mask = tf.cast(tf.where(
						sal_gamma_mask,
		...
		# Optimiser
		step = tf.compat.v1.Variable(0, trainable=False)
		self.opt = gradients(
			opt=tf.compat.v1.train.AdamOptimizer(learning_rate=params.learning_rate),
...
class DocNADE_TL_reload(object):
	def __init__(self, x, y, seq_lengths, params, W_initializer=None,
		...
		if params['use_embeddings_prior']:
			self.embeddings_lambda_list = tf.compat.v1.Variable(
